"""
Сервис аутентификации и управления учётными данными.

Отвечает за:
- регистрацию новых пользователей с проверкой уникальности email
  и отправкой приветственного письма;
- логин по паре email/пароль с выдачей access + refresh токенов;
- обновление пары токенов по валидному refresh-токену;
- смену пароля с проверкой старого и запретом на повтор.

Бизнес-правила безопасности (одинаковое сообщение при неверном
email/пароле, проверка типа refresh-токена, обязательное отличие
нового пароля от старого) сосредоточены здесь, чтобы роутеры
оставались тонкими.
"""
