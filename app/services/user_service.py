...
Сейчас минимальный: получение пользователя по id (с 404, если нет)
и обновление профиля текущего юзера (имя, email). Смена пароля
вынесена в auth_service — там она логически находится рядом с
другими операциями над учётными данными.
"""
