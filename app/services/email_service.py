"""
Сервис email-уведомлений (заглушка для учебного проекта).

Сейчас не отправляет реальные письма, а просто пишет лог-сообщение
с содержимым письма. Это оставляет место для интеграции с реальным
провайдером (SMTP, SendGrid, Mailgun) без правок вызывающего кода —
сигнатуры функций уже соответствуют рабочему API.
